import type { AppState, Catalog, StoreInfo, Order, Account, ServiceCase, Coupon, RemakeAppointment } from '../shared/types'
...
function svgPhoto(label: string, bg: string, fg = '#ffffff'): string {
...
function appealOrder(no: {
  id: string; code: string; daysAgo: number; hour: number; storeId: string;
  phone: string; contact: string; bpName: string; bpRel: string
  styleId: string; styleNote: string; inscription: string; needCold: boolean
  price: number; cake: Partial<Order['cake']>; pickupDaysAgo?: number
}): Order {
  return demoOrder({
    id: no.id,
    pickupCode: no.code,
...
const remake1006: RemakeAppointment = {
...
export function buildInitialState(): AppState {
  ...
  const orders = JSON.parse(JSON.stringify([...demoOrders, OD1004, OD1005, OD1006, OD1007, OD1008]))
