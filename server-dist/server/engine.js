...
exports.deliveryMethodText = deliveryMethodText;
exports.openStyleAppeal = openStyleAppeal;
exports.csDecideAppeal = csDecideAppeal;
exports.closeAppeal = closeAppeal;
...
// ---------- 取货后造型申诉 ----------
function deliveryMethodText(o) {
    return o.cake.needColdChain ? '到店自提 · 冷藏运输（门店提供保温袋/冰袋）' : '到店自提 · 常温携带';
}
let appealSeq = 0;
function openStyleAppeal(o, r, actor) {
    if (o.status !== 'picked_up')
        throw new ActionError('只有已签收（取货后）订单可以发起造型申诉');
    if (!o.pickedUpAt)
        throw new ActionError('缺少签收时间，无法发起申诉');
    const dl = afterSaleDeadline(o);
    if (Date.now() > new Date(dl).getTime())
        throw new ActionError(`售后窗口已于 ${fmt(dl)} 关闭，无法发起造型申诉`);
    if (!r.evidencePhoto)
        throw new ActionError('请上传取货后的问题照片，便于客服对比下单参考与门店成品照');
    if (!r.detail.trim())
        throw new ActionError('请描述造型不符的具体情况');
    if (o.appeals.some(a => a.status === 'open'))
        throw new ActionError('本单已有一条待客服判定的造型申诉，请等待处理结果');
    const a = {
        id: uid('ap'),
        orderId: o.id,
        status: 'open',
...
const STORE_SIDE = ['store', 'transport_store'];
// 客服判定：退款 / 补做 / 优惠券 / 拒绝赔付
function csDecideAppeal(o, appealId, d, actor) {
    const state = (0, store_1.getState)();
    const a = o.appeals.find(x => x.id === appealId);
    if (!a)
        throw new ActionError('申诉不存在');
    if (a.status !== 'open')
        throw new ActionError('该申诉已判定，不可重复处理');
    // 责任与判决的一致性约束
    ...
    if (!d.note.trim())
        throw new ActionError('请填写判定说明，会同步给顾客');
    ...
    if (d.verdict === 'refund') {
        ...
        if (amount <= 0)
            throw new ActionError('退款金额必须大于 0');
        if (amount > o.paidAmount + (a.refundAmount || 0))
            throw new ActionError('退款金额不能超过本单实际支付余额');
        const fee = {
    ...
    if (d.verdict === 'coupon') {
        ...
        if (amount <= 0)
            throw new ActionError('优惠券面额必须大于 0');
    ...
    if (d.verdict === 'remake') {
        if (!d.pickupDate || !d.slot)
            throw new ActionError('补做必须与顾客约定新的取货日期与时段');
        if (d.pickupDate < new Date().toISOString().slice(0, 10))
            throw new ActionError('补做取货日期不能早于今天');
        ...
        const newCode = (0, store_1.nextPickupCode)();
        a.remake = {
...
function closeAppeal(o, appealId, note, actor) {
    const a = o.appeals.find(x => x.id === appealId);
    if (!a)
        throw new ActionError('申诉不存在');
    a.status = 'closed';
...
function listCoupons(state, phone) {
...
function analytics(state) {
    ...
    const storeAtFault = (a) => !!a.responsibility && ['store', 'transport_store'].includes(a.responsibility);
    ...
    // 责任分布与判决分布（全局）
    const responsibilityDist = {};
    const verdictDist = {};
    for (const { a } of decidedAppeals) {
        if (a.responsibility)
            responsibilityDist[a.responsibility] = (responsibilityDist[a.responsibility] || 0) + 1;
        if (a.verdict)
